<template>
    <div>
<!--        <q-page-container class="row q-pt-md">-->
<!--            <div class="col-2">-->
<!--                <q-icon color="primary" size="md" name="arrow_back"/>-->
<!--            </div>-->
<!--            <div class="col q-ml-xl">-->
<!--                <p class="text-h4 text-uppercase text-weight-medium">Job Details</p>-->
<!--            </div>-->
        </q-page-container>
        <q-page-container>
            <q-page class="q-pa-lg">
                <p class="text-h5 text-weight-medium">Patient Transporter</p>
                <p class="text-subtitle1">Mercy health</p>
                <p>Qualified candidates may be considered for any
                    open position within Patient Transportation. The
                    Patient Transporter will transport patient to and from
                    the Radiology department as assigned. Keep the
                    patient’s wheelchairs and carts clean and neat.</p>
                <q-btn outline rounded/>
            </q-page>
        </q-page-container>
    </div>
</template>
<script>

</script>

<style scoped lang="scss">
    .col p {
        font-size: 20px;
    }

</style>
